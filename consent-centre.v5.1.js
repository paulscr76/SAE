...
(() => {
  ...
  const allKeys=[PLAN_KEY,OS_KEY,PREF_KEY,'saveEarnPaulV31Plan','saveEarnPaulV3Plan','saveEarnPaulV5BillMeta','saveEarnPaulV3BillMeta','saveEarnPaulRecentGuide'];
  ...
  document.getElementById('encryptedExport').onclick=async()=>{
    ...
    try{
      const payload={type:'SaveAndEarnWithPaulEncryptedBackup',version:'5.1',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
  ...
  document.getElementById('plainExport').onclick=()=>{
    const payload={type:'SaveAndEarnWithPaulPlan',version:'5.1',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
