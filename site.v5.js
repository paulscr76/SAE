...
// Version 5.0 local-first preferences and migration
(() => {
  const PREF_KEY = 'saveEarnPaulV5Preferences';
  const PLAN_KEY = 'saveEarnPaulV5Plan';
  const legacyKeys = ['saveEarnPaulV4Plan','saveEarnPaulV31Plan','saveEarnPaulV3Plan'];
  ...
  window.saveV4Preferences = (next) => {
    const prefs = {...getPrefs(), ...next};
    localStorage.setItem(PREF_KEY, JSON.stringify(prefs));
    applyPrefs();
    window.dispatchEvent(new CustomEvent('v4preferenceschange',{detail:prefs}));
    return prefs;
  };
  window.getV4Preferences = getPrefs;
