(() => {
  ...
  function meetingBrief(s, energy, actions) {
    ...
    return `Main priority: ${priorityLabel}. Eligible services to discuss: ${areas}. Energy: ${energy.actual ? 'actual annual kWh available' : 'indicative estimate only'}. Broadband contract: ${s.broadbandContract || 'unknown'}. Insurance: ${s.insuranceInterest || 'not selected'}, renewal ${s.insuranceRenewal || 'unknown'}.`;
  }
  ...
  function renderDashboard() {
    ...
    const cards = [
      card(energyKind,'Energy',energy.actual?'Actual annual kWh is available.':'This is an indicative household estimate.',`${fmt(energy.e)}${energy.g ? ' / '+fmt(energy.g) : ''}`,2,'<a href="/calculator">Explore energy scenarios</a>'),
  ...
  clear.onclick = () => {
    if (confirm('Clear the saved household plan from this device?')) {
      localStorage.removeItem(KEY); localStorage.removeItem(LEGACY_KEY); localStorage.removeItem('saveEarnPaulV31Plan'); localStorage.removeItem('saveEarnPaulV3Plan'); localStorage.removeItem('saveEarnPaulV5BillMeta');
