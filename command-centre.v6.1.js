(() => {
  ...
  function updateEnergyMessage() {
    ...
    if (quality.warnings.length) {
      ...
      box.innerHTML = `<strong>${esc(quality.label)}</strong><p>${esc(quality.confidence)}. ${quality.mode === 'estimate' ? 'The final snapshot will label this as an estimate.' : 'The final snapshot will preserve this source label.'}</p>`;
    }
  }
  ...
  function exportPlan() {
    const payload = {type:'SaveAndEarnWithPaulPlan',version:'6.1',exportedAt:new Date().toISOString(),data:readPlan()};
  ...
  form.addEventListener('change', event => {
    if (event.target.matches('[name="energyMode"]')) updateEnergyVisibility();
    updateEnergyMessage();
    savePlan();
  });
