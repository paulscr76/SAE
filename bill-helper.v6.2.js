(() => {
  ...
  document.getElementById('saveBill').addEventListener('click',()=>{
    ...
    let state={};try{state=JSON.parse(localStorage.getItem('saveEarnPaulV6Plan'))||JSON.parse(localStorage.getItem('saveEarnPaulV5Plan'))||JSON.parse(localStorage.getItem('saveEarnPaulV31Plan'))||JSON.parse(localStorage.getItem('saveEarnPaulV3Plan'))||{};}catch(_){}
    state.version='6.2';state.meta={...(state.meta||{}),updatedAt:new Date().toISOString()};state.energyMode='actual';if(e)state.actualElectric=String(e);if(g)state.actualGas=String(g);
    localStorage.setItem('saveEarnPaulV6Plan',JSON.stringify(state));
    localStorage.setItem('saveEarnPaulV6BillMeta',JSON.stringify({electricRate:document.getElementById('electricRate').value,gasRate:document.getElementById('gasRate').value,standingCharge:document.getElementById('standingCharge').value,contractEnd:document.getElementById('contractEnd').value}));
  ...
  try{
    const meta=JSON.parse(localStorage.getItem('saveEarnPaulV6BillMeta'))||JSON.parse(localStorage.getItem('saveEarnPaulV5BillMeta'))||JSON.parse(localStorage.getItem('saveEarnPaulV3BillMeta'));
    if(meta){['electricRate','gasRate','standingCharge','contractEnd'].forEach(id=>{if(meta[id]!==undefined&&document.getElementById(id))document.getElementById(id).value=meta[id]});}
  }catch(_){}
