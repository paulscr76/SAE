(() => {
  const KEY = 'saveEarnPaulV31Plan';
  const LEGACY_KEY = 'saveEarnPaulV3Plan';
  const VERSION = '3.1';
  ...
  function reportHtml(s, energy, readinessInfo, actions) {
    ...
    const cards = actions.map((a,i) => `<div class="action"><b>${i+1}</b><div><strong>${a.title}</strong><p>${a.detail}</p></div></div>`).join('');
    const outstanding = readinessInfo.outstanding.length ? readinessInfo.outstanding.join(', ') : 'Nothing essential identified';
    return `<!doctype html><html><head><meta charset="utf-8"><title>Household Action Plan</title><style>body{font-family:Arial,sans-serif;color:#10243f;margin:0;background:#f3f7fc}.wrap{max-width:850px;margin:30px auto;background:#fff;padding:40px;border-radius:24px}.head{background:#02172f;color:#fff;border-radius:20px;padding:28px}.head h1{color:#ffd60a;margin:5px 0}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin:18px 0}.card,.action{border:1px solid #d9e5f2;border-radius:15px;padding:15px}.card strong{display:block;font-size:20px}.action{display:grid;grid-template-columns:38px 1fr;gap:12px;margin:10px 0}.action b{width:38px;height:38px;border-radius:12px;background:#ffd60a;display:grid;place-items:center}.action p{margin:5px 0;color:#667890}.footer{margin-top:25px;border-top:1px solid #d9e5f2;padding-top:18px;font-size:12px;color:#667890}.print{margin:0 0 15px;padding:12px 16px;background:#ffd60a;border:0;border-radius:10px;font-weight:bold}@media print{.print{display:none}.wrap{margin:0;box-shadow:none}}</style></head><body><div class="wrap"><button class="print" onclick="print()">Print or save as PDF</button><div class="head"><small>SAVE &amp; EARN WITH PAUL</small><h1>Household Action Plan</h1><p>${priorityLabel}</p></div><div class="grid"><div class="card"><small>INFORMATION READINESS</small><strong>${readinessInfo.complete} of ${readinessInfo.total} areas</strong></div><div class="card"><small>ENERGY CONFIDENCE</small><strong>${readinessInfo.energyConfidence}</strong></div><div class="card"><small>ELECTRICITY</small><strong>${fmt(energy.e)}</strong></div><div class="card"><small>GAS</small><strong>${energy.g ? fmt(energy.g) : 'Not applicable'}</strong></div></div><h2>Ordered next steps</h2>${cards}<h2>Outstanding information</h2><p>${outstanding}</p><h2>Book a conversation</h2><p>Free 30-minute chat: ${CALENDLY}</p><div class="footer">Generated locally on ${new Date().toLocaleDateString('en-GB',{dateStyle:'long'})}. Guidance only—not a quotation, supplier recommendation, financial advice or income forecast. Paul Scrase is an independent Utility Warehouse Partner.</div></div></body></html>`;
  }
  ...
  async function importPlan(file) {
    try {
      ...
      error.innerHTML = '<strong>That file could not be imported.</strong><br>Choose a Version 3.1 household plan JSON file.';
  ...
  function renderDashboard() {
    ...
    document.getElementById('visualReport').onclick = () => { const win = window.open('','_blank'); if (win) { win.document.write(reportHtml(s,energy,readinessInfo,actions)); win.document.close(); } window.trackSafe?.('visual_report'); };
  ...
  clear.onclick = () => {
    if (confirm('Clear the saved household plan from this device?')) {
      localStorage.removeItem(KEY); localStorage.removeItem(LEGACY_KEY); localStorage.removeItem('saveEarnPaulV3BillMeta');
