(() => {
  const KEY = 'saveEarnPaulV4Plan';
  const LEGACY_KEY = 'saveEarnPaulV31Plan';
  const VERSION = '4.0';
  ...
  clear.onclick = () => {
    if (confirm('Clear the saved household plan from this device?')) {
      localStorage.removeItem(KEY); localStorage.removeItem(LEGACY_KEY); localStorage.removeItem('saveEarnPaulV3Plan'); localStorage.removeItem('saveEarnPaulV3BillMeta');
