...
(() => {
  ...
  const defaultOS=()=>({version:'4.0',tasks:[],documents:[],consent:{energy:true,broadband:true,mobile:true,earning:false,tasks:true,documents:false},updatedAt:new Date().toISOString()});
  ...
  const energy=()=>{
    ...
    if(plan.energyMode==='actual')return{e:+plan.actualElectric||e,g:plan.heating==='gas'?(+plan.actualGas||g):0,actual:Boolean(+plan.actualElectric||+plan.actualGas)};
    return{e,g,actual:false};
  };
  ...
  function renderOverview(){
    ...
    const en=energy(),open=os.tasks.filter(t=>t.status!=='done').length,done=os.tasks.filter(t=>t.status==='done').length;
    const complete=[plan.priority,plan.propertyType,plan.energyMode,plan.broadbandExperience!=='unknown',plan.mobileLines==='none'||plan.mobileConcern!=='none',plan.earningInterest==='none'||plan.earningTime!=='unknown'].filter(Boolean).length;
    const energyStatus=en.actual?'Actual figures':'Estimated';
    ...
    root.innerHTML=`<div class="os-card"><div class="os-head"><div><span class="kicker">Welcome back</span><h1>${priorityLabel(plan.priority)}</h1><p>Your household snapshot was last updated ${plan.meta?.updatedAt?new Date(plan.meta.updatedAt).toLocaleString('en-GB'): 'on this device'}.</p></div><span class="local-pill">Not sent to Paul</span></div>
      <div class="os-metrics"><div class="os-metric"><span>Information readiness</span><strong>${complete} of 6</strong></div><div class="os-metric"><span>Open actions</span><strong>${open}</strong></div><div class="os-metric"><span>Completed actions</span><strong>${done}</strong></div><div class="os-metric"><span>Energy confidence</span><strong>${energyStatus}</strong></div></div>
      <div class="os-launch-actions"><a class="btn btn-blue" href="/command-centre">Edit household details</a><button class="btn btn-light" id="overviewTasks" type="button">Open living plan</button><a class="btn btn-light" href="/copilot">Ask the Copilot</a></div></div>
      ...
      <article class="os-domain"><div class="os-domain-top"><h3>Energy</h3><span class="os-status ${en.actual?'ready':'review'}">${en.actual?'Ready':'Review'}</span></div><p>${fmt(en.e)} electricity${en.g?' and '+fmt(en.g)+' gas':''}. ${en.actual?'Annual kWh supplied.':'Indicative estimate.'}</p><a href="/calculator">Open scenario lab →</a></article>
  ...
  function brief(selections=os.consent){
    ...
    if(selections.energy)parts.push(`Energy: ${fmt(en.e)} electricity${en.g?' and '+fmt(en.g)+' gas':''} (${en.actual?'annual figures supplied':'indicative estimate'}).`);
  ...
  const prefs=window.getV4Preferences?.()||{};
  ...
  document.getElementById('themePreference').onchange=e=>window.saveV4Preferences({theme:e.target.value});
  document.getElementById('textPreference').onchange=e=>window.saveV4Preferences({textSize:e.target.value});
  document.getElementById('motionPreference').onchange=e=>window.saveV4Preferences({reduceMotion:e.target.checked});
