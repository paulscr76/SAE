...
(() => {
  ...
  const samplePlan={
    version:'6.0',priority:'both',propertyType:'semi',bedrooms:'3',occupants:'3',heating:'gas',efficiency:'average',daytime:'mixed',
  ...
  function footer(page,total,title){
    return `<div class="report-page-footer"><span><strong>Save &amp; Earn with Paul v6.0</strong> • ${esc(title)}</span><span>Confidential household planning document • Page ${page} of ${total}</span></div>`;
  }
  ...
  function render(type){
    ...
    root.innerHTML=html;
    document.title=`${type==='energy'?'Energy Data Passport':type==='actions'?'Action Plan':type==='brief'?'Meeting Brief':'Complete Household'} Report | Save & Earn with Paul`;
  }
