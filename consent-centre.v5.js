...
(() => {
  const PLAN_KEY='saveEarnPaulV5Plan',OS_KEY='saveEarnPaulV5OS',PREF_KEY='saveEarnPaulV5Preferences';
  const allKeys=[PLAN_KEY,OS_KEY,PREF_KEY,'saveEarnPaulV31Plan','saveEarnPaulV3Plan','saveEarnPaulV3BillMeta','saveEarnPaulRecentGuide'];
  ...
  let consent={energy:true,broadband:true,mobile:true,earning:false,tasks:true,documents:false,...(os().consent||{})};
  ...
  document.getElementById('encryptedExport').onclick=async()=>{
    ...
    try{
      const payload={type:'SaveAndEarnWithPaulEncryptedBackup',version:'5.0',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
  ...
  document.getElementById('plainExport').onclick=()=>{
    const payload={type:'SaveAndEarnWithPaulPlan',version:'5.0',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
    const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='save-and-earn-v4-plan.json';a.click();URL.revokeObjectURL(a.href);
  };
  ...
  document.getElementById('eraseV4Data').onclick=()=>{
    if(!confirm('Erase all Save & Earn local data from this browser?'))return;
    allKeys.forEach(k=>localStorage.removeItem(k));Object.keys(localStorage).filter(k=>k.startsWith('guideFeedback:')).forEach(k=>localStorage.removeItem(k));renderInventory();updateBrief();showToast('Local Household OS data erased');
  };
  renderInventory();
})();
