...
(() => {
  const PLAN_KEY='saveEarnPaulV4Plan',OS_KEY='saveEarnPaulV4OS',PREF_KEY='saveEarnPaulV4Preferences';
  ...
  document.getElementById('encryptedExport').onclick=async()=>{
    ...
    try{
      const payload={type:'SaveAndEarnWithPaulEncryptedBackup',version:'4.0',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
  ...
  document.getElementById('plainExport').onclick=()=>{
    const payload={type:'SaveAndEarnWithPaulPlan',version:'4.0',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
