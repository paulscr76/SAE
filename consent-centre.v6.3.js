...
(() => {
  ...
  document.getElementById('encryptedExport').onclick=async()=>{
    ...
    try{
      const payload={type:'SaveAndEarnWithPaulEncryptedBackup',version:'6.3',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
  ...
  document.getElementById('plainExport').onclick=()=>{
    const payload={type:'SaveAndEarnWithPaulPlan',version:'6.3',exportedAt:new Date().toISOString(),plan:plan(),os:os(),preferences:read(PREF_KEY)};
