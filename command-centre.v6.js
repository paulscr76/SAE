(() => {
  const KEY = 'saveEarnPaulV6Plan';
  const LEGACY_KEY = 'saveEarnPaulV5Plan';
  const VERSION = '6.0';
  ...
  const ids = ['propertyType','bedrooms','occupants','heating','efficiency','daytime','energyMode','energyConcern','actualElectric','actualGas','ev','solar','broadbandExperience','broadbandContract','workFromHome','mobileLines','mobileConcern','mobileContract','insuranceInterest','insuranceRenewal','earningInterest','earningTime','earningGoal'];
  ...
  function load() {
    try {
      ...
      if (!state) {
        state = JSON.parse(localStorage.getItem(LEGACY_KEY)) || JSON.parse(localStorage.getItem('saveEarnPaulV4Plan'));
  ...
  function energyValues(s = rawState()) {
    ...
    if (['actual','annualised','epc'].includes(s.energyMode)) {
      const insight=s.meta?.energyInsight;
      return {
        e:+s.actualElectric || estimated.e,
        g:s.heating === 'gas' ? (+s.actualGas || estimated.g) : 0,
        actual:s.energyMode==='actual' && (insight ? Boolean(insight.actual) : Boolean(+s.actualElectric || +s.actualGas)),
        sourceLabel:insight?.sourceLabel || (s.energyMode==='annualised'?'Meter-reading annualisation':s.energyMode==='epc'?'EPC-informed household estimate':'Annual kWh supplied'),
        confidence:insight?.confidence || (s.energyMode==='actual'?'High confidence':s.energyMode==='annualised'?'Medium confidence':'Medium confidence')
      };
    }
    return {...estimated, actual:false,sourceLabel:'Transparent household estimate',confidence:'Indicative'};
  }

  const fmt = n => `${Math.round(n).toLocaleString('en-GB')} kWh`;
  function toggleActual() {
    const showActual = document.getElementById('energyMode').value !== 'estimate';
  ...
  function updateContextTips() {
    ...
    const tips = [
      ...
      s.energyMode === 'actual' ? ((+s.actualElectric || +s.actualGas) ? 'Annual kWh from a bill or supplier gives the strongest basis. Check that the figures are usage—not cost or a meter reading.' : 'Annual-kWh mode is selected, but no figures have been entered yet.') : s.energyMode === 'annualised' ? 'These figures were annualised from household meter readings. Longer periods reduce seasonal distortion.' : s.energyMode === 'epc' ? 'Official EPC property information informed this estimate, but it is not actual meter consumption.' : 'This estimate is a planning baseline. Use the Energy Data Passport when stronger information becomes available.',
      (s.broadbandContract === 'unknown' || (s.insuranceInterest !== 'none' && s.insuranceRenewal === 'unknown')) ? 'Contract and renewal dates help Paul understand when a conversation may be useful.' : 'Your service timing gives Paul a clearer starting point.',
  ...
  function validate() {
    ...
    if (step === 2 && document.getElementById('energyMode').value !== 'estimate' && !(+document.getElementById('actualElectric').value || +document.getElementById('actualGas').value)) {
  ...
  function readiness(s, energy, actions) {
    ...
    if (s.energyMode === 'estimate' || !(+s.actualElectric || +s.actualGas)) outstanding.push('Stronger annual energy kWh');
    ...
    if (s.insuranceInterest !== 'none' && s.insuranceRenewal === 'unknown') outstanding.push('Insurance renewal timing');
    const domains = [
      Boolean(s.priority),
      Boolean(s.propertyType && s.bedrooms && s.occupants && s.heating),
      s.energyConcern === 'none' || Boolean(+s.actualElectric || +s.actualGas) || s.energyMode === 'estimate',
      s.broadbandExperience !== 'unknown' && s.broadbandContract !== 'unknown',
      s.mobileLines === 'none' || s.mobileConcern !== 'none',
      s.insuranceInterest === 'none' || s.insuranceRenewal !== 'unknown',
      s.earningInterest === 'none' || s.earningTime !== 'unknown'
    ];
    const complete = domains.filter(Boolean).length;
    const energyConfidence = `${energy.confidence || (energy.actual?'High confidence':'Indicative')} — ${energy.sourceLabel || 'energy source not supplied'}`;
  ...
  function buildActions(s, energy = energyValues(s)) {
    ...
    if (s.energyConcern !== 'none' || ['save','understand','both'].includes(s.priority)) {
      actions.push({id:'energy',title:energy.actual?'Keep your latest bill available':'Improve the energy evidence when possible',detail:energy.actual?'The household plan includes customer-supplied annual figures.':`Current basis: ${energy.sourceLabel || 'estimate'}. Use a bill or longer reading period to improve confidence.`,step:2});
    }
    ...
    const insuranceNeeds = s.insuranceInterest && s.insuranceInterest !== 'none';
    if (insuranceNeeds) actions.push({id:'insurance',title:'Check insurance renewal details',detail:'Note the type of cover, renewal date and any questions you want Paul to answer.',step:3});
  ...
  function meetingBrief(s, energy, actions) {
    ...
    return `Main priority: ${priorityLabel}. Eligible services to discuss: ${areas}. Energy: ${energy.sourceLabel || (energy.actual ? 'actual annual kWh available' : 'indicative estimate')}; ${energy.confidence || 'confidence not supplied'}. Broadband contract: ${s.broadbandContract || 'unknown'}. Insurance: ${s.insuranceInterest || 'not selected'}, renewal ${s.insuranceRenewal || 'unknown'}.`;
  }
  ...
  function renderDashboard() {
    ...
    const cards = [
      card(energyKind,'Energy',`${energy.sourceLabel || (energy.actual?'Annual kWh supplied':'Household estimate')}. ${energy.confidence || ''}`,`${fmt(energy.e)}${energy.g ? ' / '+fmt(energy.g) : ''}`,2,'<a href="/calculator">Open Energy Data Passport</a>'),
      ...
      card(insuranceNeeds?'review':'low','Insurance',insuranceNeeds?'Renewal timing and cover can be included in the conversation.':'Insurance is not currently selected.','',3),
    ...
    document.getElementById('jumpFirstGap').onclick = () => {
      ...
      show(first.includes('priority') ? 0 : first.includes('energy') ? 2 : first.includes('Broadband') || first.includes('Mobile') || first.includes('Insurance') ? 3 : 0);
    };
    ...
    document.getElementById('sendPlan').onclick = () => { window.open(`${WHATSAPP}?text=${encodeURIComponent('Hi Paul, I completed my household plan. '+brief+' Suggested actions: '+actions.map(a=>a.title).join('; ')+'. Could we discuss it?')}`,'_blank','noopener'); window.trackSafe?.('command_whatsapp'); };
    document.getElementById('visualReport').onclick = () => { location.href='/report-studio?type=full'; window.trackSafe?.('printable_summary_open',{type:'full'}); };
  ...
  clear.onclick = () => {
    if (confirm('Clear the saved household plan from this device?')) {
      localStorage.removeItem(KEY); localStorage.removeItem(LEGACY_KEY); localStorage.removeItem('saveEarnPaulV31Plan'); localStorage.removeItem('saveEarnPaulV3Plan'); localStorage.removeItem('saveEarnPaulV6BillMeta');
