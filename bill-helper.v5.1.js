(() => {
  ...
  document.getElementById('saveBill').addEventListener('click',()=>{
    ...
    state.version='5.1';state.meta={...(state.meta||{}),updatedAt:new Date().toISOString()};state.energyMode='actual';if(e)state.actualElectric=String(e);if(g)state.actualGas=String(g);
    localStorage.setItem('saveEarnPaulV5Plan',JSON.stringify(state));
    localStorage.setItem('saveEarnPaulV5BillMeta',JSON.stringify({electricRate:document.getElementById('electricRate').value,gasRate:document.getElementById('gasRate').value,standingCharge:document.getElementById('standingCharge').value,contractEnd:document.getElementById('contractEnd').value}));
  ...
  try{
    const meta=JSON.parse(localStorage.getItem('saveEarnPaulV5BillMeta'))||JSON.parse(localStorage.getItem('saveEarnPaulV3BillMeta'));
